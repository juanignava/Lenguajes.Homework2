...
obtenerCiudad([_|List], C) :- obtenerCiudad(List, C).
...
obtenerRespuesta([_|List], R) :- obtenerRespuesta(List, R).
...
% Analisis sobre las gramaticas libres de contexto.

% Nombre: Oracion 1.
% Descripcion: analiza si la oracion contiene la sintaxis correcta.
